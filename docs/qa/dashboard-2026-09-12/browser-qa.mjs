...
try {
    ...
    await check('ROOM-04', 'Room gallery rejects non-image uploads', async () => {
        await go('/admin/rooms/create');
        const response = await submit('/admin/rooms', {...roomData,name:'QA Invalid Gallery','gallery_images[0]':{name:'qa-not-image.txt',mimeType:'text/plain',buffer:Buffer.from('Harmless QA upload; not an image.')}}, true);
        const id = await idFor('rooms','QA Invalid Gallery');
        const edit = await page.request.get(base+`/admin/rooms/${id}/edit`);
        assert.ok(!(await edit.text()).includes('QA Invalid Gallery'), 'Non-image gallery upload was accepted and a room was created.');
    ...
    await check('PACKAGE-02', 'Package maximum guests cannot be below minimum', async () => {
        ...
        await submit(`/admin/packages/${packageId}`,{name:'QA Weekend Package',min_guests:'5',max_guests:'2',sort_order:'0',is_active:'1',_method:'PUT'});
        await go(`/admin/packages/${packageId}/edit`);
        assert.ok(Number(await page.locator('[name=max_guests]').inputValue()) >= Number(await page.locator('[name=min_guests]').inputValue()),'Saved minimum 5 with maximum 2.');
    });
    ...
    for (const section of ['rooms','packages','experiences','services','testimonials','blogs','enquiries']) {
        await check('MOBILE-'+section,'Mobile '+section+' actions are reachable without clipping',async()=>{
            ...
            const clipped=await page.locator('table').evaluateAll(tables=>tables.flatMap(table=>[...table.querySelectorAll('a,button')].filter(el=>{
                ...
                while(parent){ const c=getComputedStyle(parent); const b=parent.getBoundingClientRect(); if(c.overflowX==='hidden' && (rect.right>b.right+1 || rect.left<b.left-1)) return true; parent=parent.parentElement; }
                return false;
