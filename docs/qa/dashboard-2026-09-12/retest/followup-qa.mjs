...
try{
 ...
 await check('GALLERY-04','Gallery metadata edit persists without replacing image',async()=>{
  await go('/admin/gallery'); const card=page.locator('.group.relative').filter({has:page.getByText('QA photo',{exact:true})});
  const source=await card.locator('img').getAttribute('src');await card.getByRole('link',{name:'Edit',exact:true}).click();
  await page.locator('[name=title]').fill('QA photo edited');await page.locator('[name=badge_label]').fill('Dining');await page.locator('[name=section]').selectOption('dining');await page.locator('[name=sort_order]').fill('7');
  await page.getByRole('button',{name:'Save Changes',exact:true}).click();await page.waitForURL('**/admin/gallery');
  const edited=page.locator('.group.relative').filter({has:page.getByText('QA photo edited',{exact:true})});assert.equal(await edited.locator('img').getAttribute('src'),source);
  await edited.getByRole('link',{name:'Edit',exact:true}).click();assert.equal(await page.locator('[name=section]').inputValue(),'dining');assert.equal(await page.locator('[name=badge_label]').inputValue(),'Dining');
 });
 ...
 await check('UX-10','Mobile focus stays in open navigation and restores on close',async()=>{
  const close=page.locator('[data-sidebar-close]');await close.focus();await page.keyboard.press('Shift+Tab');assert.equal(await page.evaluate(()=>document.activeElement.id),'btn-logout');
  await page.keyboard.press('Tab');assert.equal(await close.evaluate(el=>el===document.activeElement),true);
  await page.keyboard.press('Escape');assert.equal(await page.evaluate(()=>document.activeElement.id),'sidebar-toggle');
  await page.setViewportSize({width:1440,height:1000});assert.equal(await page.locator('#sidebar').evaluate(el=>el.inert),false);assert.equal(await page.locator('#admin-content').evaluate(el=>el.inert),false);
 });
